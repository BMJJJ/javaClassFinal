...
public class ParkController {
	...
	public String parkGet(NationalVO vo, Model model, String ymd,
	 ...
	 List<String> courseVos = scheduleService.getNationNoneCourseList(ymd);
	...
	// 스케줄 달력 띄우기
		...
		public String myScheduleGet(HttpSession session, Model model) {
			String mid = (String) session.getAttribute("sMid");
		  List<ScheduleVO> vos = scheduleService.getMySchedule(mid);
		  model.addAttribute("vos", vos);
		  return "park/mySchedule";
		}

		@RequestMapping(value = "/scheduleUpdate", method = RequestMethod.GET)
		public String scheduleUpdateGet(int idx, Model model) {
			ScheduleVO vo = scheduleService.getScheduleById(idx);
		  model.addAttribute("vo", vo);
		  return "park/scheduleUpdate";
		}

		@RequestMapping(value = "/scheduleUpdate", method = RequestMethod.POST)
		public String scheduleUpdatePost(ScheduleVO vo) {
		    int res = scheduleService.setScheduleUpdate(vo);
		    if (res != 0) return "redirect:/message/scheduleUpdateOk";
		    else return "redirect:/message/scheduleUpdateNo";
		}
		
		// 스케줄 삭제하기
		@RequestMapping(value = "/scheduleDelete", method=RequestMethod.POST)
		public String scheduleDeleteGet(int idx) {
			int res = scheduleService.setScheduleDelete(idx);
			if(res != 0) return "redirect:/message/scheduleDeleteOk";
			else return "redirect:/message/scheduleDeleteNo";
		}
		...
		public String parkUpdateGet(Model model) {
		    List<NationalVO> vos = parkSerive.getParkList("전체보기");
		...
		@RequestMapping(value = "/parkUpdateForm", method = RequestMethod.GET)
		public String parkUpdateFormGet(@RequestParam("idx") int idx, Model model) {
		    NationalVO vo = parkSerive.getNationalPark(idx);
		    model.addAttribute("vo", vo);
		    return "park/parkUpdateForm";
		}

		@RequestMapping(value = "/parkUpdateForm", method = RequestMethod.POST)
		public String parkUpdateFormPost(NationalVO vo) {
		    // 이미지 처리 로직은 parkInput 메서드와 유사하게 구현
		    if(vo.getPhoto().indexOf("src=\"/") != -1) parkSerive.imgCheck(vo.getPhoto());
		    vo.setPhoto(vo.getPhoto().replace("/data/ckeditor/", "/data/park/"));
		    
		    int res = parkSerive.setParkUpdate(vo);
		    
		    if(res != 0) return "redirect:/message/parkUpdateOk";
		    else return "redirect:/message/parkUpdateNo";
		}
		
		@ResponseBody
		@RequestMapping(value = "/parkDelete", method = RequestMethod.POST)
		public String parkDeletePost(int idx) {
		    int res = parkSerive.setParkDelete(idx);
		    if(res != 0) return "redirect:/message/parkDeleteOk";
		    else return "redirect:/message/parkDeleteNo";
		}
