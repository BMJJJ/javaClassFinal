...
public class AdminController {
	...
	@RequestMapping(value = "/board/complaintList", method = RequestMethod.GET)
	public String complaintListGet(Model model,
			@RequestParam(name="pag", defaultValue = "1", required = false) int pag,
			@RequestParam(name="pageSize", defaultValue = "10", required = false) int pageSize) {
		PageVO pageVO = pageProcess.totRecCnt(pag, pageSize, "complaint", "", "");
		String complaint = "";
		ArrayList<BoardVO> vos = adminService.getComplaintList(pageVO.getStartIndexNo(), pageSize, complaint);
		
		model.addAttribute("vos", vos);
		model.addAttribute("pageVO", pageVO);
		
		return "admin/board/complaintList";
	}
