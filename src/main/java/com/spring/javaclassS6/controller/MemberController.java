...
//카카오 로그인 
	...
	public String memberMainGet(HttpSession session, Model model) {
		...
		int scCnt = memberService.getScheduleCount(mid);
		
		int qnCnt = memberService.getQnaCount(mid);
		
		model.addAttribute("mVo", mVo);
		model.addAttribute("guestCnt", guestCnt);
		model.addAttribute("boardCnt", boardCnt);
		model.addAttribute("pdsCnt", pdsCnt);
		model.addAttribute("scCnt", scCnt);
		model.addAttribute("qnCnt", qnCnt);
    ...
    if(part.equals("NewMid")) {
      ...
      content += "<h2>안녕하세요 길위의 이야기입니다.</h2><br>"
              + "<h3>길위의 이야기의 규정에 따라에 따라 귀하의 아이디 찾기 요청이 접수되었습니다.<br>"
      ...
      content = content.replace("\n", "<br>");
      content += "<h2>안녕하세요 길위의 이야기입니다.</h2><br>"
              + "<h3>길위의 이야기의 규정에 따라 귀하의 비밀번호 재설정 요청이 접수되었습니다.<br>"
      ...
      content += "관리자 드림";
		...
		FileSystemResource file = new FileSystemResource(request.getSession().getServletContext().getRealPath("/resources/images/logo2.jpg"));
