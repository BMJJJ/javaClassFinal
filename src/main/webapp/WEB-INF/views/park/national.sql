...
insert into schedule values (default,1,'산책로','우이동 경관 둘레길','admin','2024-08-31',3,'잘부탁드려요','NO');
insert into schedule values (default,2,'탐방로(해안)','정동진 부채길','admin','2024-08-30',5,'잘부탁드려요2','NO');
insert into schedule values (default,2,'탐방로(해안)','정동진 부채길','admin','2024-08-30',2,'부부가 갑니다.','NO');
insert into schedule values (default,4,'산책로','광명시 천연동굴','admin','2024-08-30',5,'친구모임으로 갑니다.','NO');
insert into schedule values (default,1,'산책로','우이동 경관 둘레길','hkd1234','2024-08-30',5,'가 볼까요?','NO');
insert into schedule values (default,1,'산책로','우이동 경관 둘레길','kms1234','2024-08-30',2,'우리도 갑니다.','NO');

select * from schedule;
select course,sum(visitCnt) as allVisitCnt from schedule where visitDate = '2024-8-30';
select course,ifnull(sum(visitCnt),0) as allVisitCnt from schedule where visitDate = '2024-8-30' group by course;
select ifnull(course,course),ifnull(sum(visitCnt),0) as allVisitCnt from (select distinct part from national) c left join schedule n on c.part = n.part group by c.part;
...
insert into noneVisit values (default, 6, '2024-08-31');
insert into noneVisit values (default, 6, '2024-09-03');
insert into noneVisit values (default, 6, '2024-09-23');
insert into noneVisit values (default, 6, '2024-10-23');
...
select date_format(noneDate,'%Y-%m-%d') as noneDate,(select part from national where idx = v.nationalIdx) as part,count(*) as partCnt from noneVisit v where date_format(noneDate,'%Y-%m')='2024-08' group by noneDate,part  order by noneDate,part;

select date_format(noneDate,'%Y-%m-%d') as noneDate,count(*) as partCnt from noneVisit where date_format(noneDate,'%Y-%m')='2024-08' group by noneDate  order by noneDate;
...
select *,(select count(*) from noneVisit where date_format(noneDate, '%Y-%m-%d')='2024-8-1') as visitSw from national order by idx desc;

select * from noneVisit n where noneDate = '2024-8-1';
select (select course from national where idx=n.nationalIdx) as course from noneVisit n where noneDate = '2024-8-1';